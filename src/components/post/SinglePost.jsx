...
const SinglePost = () => {
  ...
  useEffect(() => {
    const fetchMostLikedComments = async () => {
      const commentResp = await axios.get(
        rootAPI + `comment/getCommentsByMostLiked/${post._id}`
      );
      dispatch(getMostLikedCommentsByPostId(post._id)).then(() => {
                ...
                <div className="row">
                  ...
                  <div className="col-6">
                    <div className="suggested-price">
                      {comments ? (
                        <p>IDR{comments[0]?.suggestedPrice || "0"}</p>
                      ) : (
                        //   <p>none</p>
                        <MiniSpinner />
                      )}
